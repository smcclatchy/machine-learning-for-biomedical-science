...
library(knitr)
opts_chunk$set(fig.path=paste0("fig/", sub("(.*).Rmd","\\1",basename(knitr:::knit_concord$get('infile'))), "-"))
```
...
```{r conditional_prob, fig.cap="Probability of Y=1 as a function of X1 and X2. Red is close to 1, yellow close to 0.5, and blue close to 0.", eval=FALSE}
library(rafalib)
...
```{r bayes_rule,fig.cap="Bayes rule. The line divides part of the space for which probability is larger than 0.5 (red) and lower than 0.5 (blue).", eval=FALSE}
mypar()
...
```{r test_train, fig.cap="Training data (left) and test data (right).", fig.width=10.5, fig.height=5.25, eval=FALSE}
...
```{r regression_prediction, fig.cap="We estimate the probability of 1 with a linear regression model with X1 and X2 as predictors. The resulting prediction map is divided into parts that are larger than 0.5 (red) and lower than 0.5 (blue).", eval=FALSE}
colshat <- yhat
