...
If we show points for which $E(Y \mid X=x)>0.5$ in red and the rest in blue, we 
see the boundary region that denotes the boundary in which we switch from 
predicting 0 to 1.

```{r bayes_rule,fig.cap="Bayes rule. The line divides part of the space for which probability is larger than 0.5 (red) and lower than 0.5 (blue)."}
mypar()
...
Let's download and then load RData file that will enable us to run following 
code.
...
ml-biomed-classpredict-setup-20221019.RData may differ depending on where you 
have placed RData file. Adjust path accordingly in the command below.

```{r load data for class prediction}
